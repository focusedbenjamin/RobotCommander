...
alias robot='echo $ROBOT_HOSTNAME'
alias master='echo $ROBOT_HOSTNAME'
...
alias websocket_address='echo "wss://$(instance_id).robotigniteacademy.com/${SLOT_PREFIX}/websocket/12000/"' # 12000-12007
alias crash-detector-enable='set -x; export LD_PRELOAD=/usr/local/lib/theconstruct/crash_detector.so; set +x'
alias crash-detector-disable='set -x; unset LD_PRELOAD; set +x'
...
if [ -n "$ROBOT_DESCRIPTION_PATH" ]; then
    # Sentinel - load Robot Model in Rviz
    export AMENT_PREFIX_PATH="$AMENT_PREFIX_PATH:$ROBOT_DESCRIPTION_PATH"
fi
